...
const TsComponent = () => {
  ...
  // useState에 초기 값을 넣어줄 경우, 초기 값의 타입으로 알아서 해석함
  const [inputTitle, setInputTitle] = useState('');
  const [inputSearch, setInputSearch] = useState('');
  const [comments, setComment] = useState<commentRow[]>([
    {
      writer: '민수',
      title: '안뇽',
    },
  ...
  const searchComment = () => {
    let searchResult = comments.filter((item) => {
  ...
  return (
    <div>
      ...
      <h3>댓글 검색 결과</h3>
      {result.length > 0 ? <CommentTable comments={result} /> : <h5>검색 결과가 없습니다.</h5>}
    </div>
  );
};
